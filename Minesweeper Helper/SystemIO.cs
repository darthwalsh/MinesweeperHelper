...
namespace Minesweeper_Helper
{
    ...
    class SystemIO
    {
        ...
        private int[,] hints; //Numbers that have already been checked

        private OCR myOCR; //Character recognition

        private bool gameFinished = false; //Whether the appears to be over

        //Game options
        private bool DISTINGUISH_378 = true;

        public SystemIO(bool STEP = false)
        {
            ...
            Bitmap filtered = new Bitmap(1, 1);
            //DEBUG take screen shot of World
            if (STEP)
            {
                filtered = new Bitmap(scr);
                for (int x = 0; x < filtered.Width; ++x)
                    for (int y = 0; y < filtered.Height; ++y)
                        filtered.SetPixel(x, y, (isWhiteUL(filtered.GetPixel(x, y)) ? Color.White : Color.Black));
                GDI32.SaveImage(filtered);
            }
            ...
            while (colHeight < 130)
            {
                ...
                while (!isWhiteUL(getPixel(pixelIndex)))
                {
                    ...
                    if (STEP)
                    {
                        filtered.SetPixel(pixelIndex.X - 1, pixelIndex.Y, Color.Red);
                        filtered.SetPixel(pixelIndex.X, pixelIndex.Y, Color.Red);
                        filtered.SetPixel(pixelIndex.X + 1, pixelIndex.Y, Color.Red);
                    }
                }
                ...
                while (!isWhiteUL(getPixel(pixelIndex)))
                {
                    ...
                    if (STEP)
                    {
                        filtered.SetPixel(pixelIndex.X - 1, pixelIndex.Y, Color.Orange);
                        filtered.SetPixel(pixelIndex.X, pixelIndex.Y, Color.Orange);
                        filtered.SetPixel(pixelIndex.X + 1, pixelIndex.Y, Color.Orange);
                    }
                }
            }
                   ...
                   !isWhiteUL(getPixel(twoup)))
            {
                ...
                if (STEP)
                {
                    filtered.SetPixel(pixelIndex.X - 1, pixelIndex.Y, Color.Yellow);
                    filtered.SetPixel(pixelIndex.X, pixelIndex.Y, Color.Yellow);
                    filtered.SetPixel(pixelIndex.X + 1, pixelIndex.Y, Color.Yellow);
                }
            }
            ...
            while (!isWhiteUL(getPixel(pixelIndex)))
            {
                ...
                if (STEP)
                {
                    filtered.SetPixel(pixelIndex.X, pixelIndex.Y - 1, Color.Green);
                    filtered.SetPixel(pixelIndex.X, pixelIndex.Y, Color.Green);
                    filtered.SetPixel(pixelIndex.X, pixelIndex.Y + 1, Color.Green);
                }
            }
            ...
            while (!isWhiteUL(getPixel(pixelIndex)))
            {
                ...
                if (STEP)
                {
                    filtered.SetPixel(pixelIndex.X, pixelIndex.Y - 1, Color.Blue);
                    filtered.SetPixel(pixelIndex.X, pixelIndex.Y, Color.Blue);
                    filtered.SetPixel(pixelIndex.X, pixelIndex.Y + 1, Color.Blue);
                }
            }
            ...
            if (STEP)
            {
                ...
                for (int x = 0; x < filtered.Width; ++x)
                    for (int y = 0; y < filtered.Height; ++y)
                        filtered.SetPixel(x, y, (isWhiteLR(filtered.GetPixel(x, y)) ? Color.White : Color.Black));
                GDI32.SaveImage(filtered);
            }
            ...
            {                                    //to be to the left one
                ...
                if (STEP)
                {
                    filtered.SetPixel(pixelIndex.X, pixelIndex.Y - 1, Color.Red);
                    filtered.SetPixel(pixelIndex.X, pixelIndex.Y, Color.Red);
                    filtered.SetPixel(pixelIndex.X, pixelIndex.Y + 1, Color.Red);
                }
            }
            ...
            if (STEP)
            {
                ...
                for (int x = 0; x < filtered.Width; ++x)
                    for (int y = 0; y < filtered.Height; ++y)
                        filtered.SetPixel(x, y, (isWhite(filtered.GetPixel(x, y), 40) ? Color.White : Color.Black));
                GDI32.SaveImage(filtered);
            }
            ...
            if (STEP)
            {
                for (double X = leftCol; X <= leftCol + numWidth * cellDimDbl + 1; X += cellDimDbl)
                    for (int y = topHeight; y <= bottomHeight; ++y)
                        filtered.SetPixel((int)X, y, Color.Red);
                for (double Y = topHeight; Y <= topHeight + numHeight * cellDimDbl + 1; Y += cellDimDbl)
                    for (int x = leftCol; x <= rightCol; ++x)
                        filtered.SetPixel(x, (int)Y, Color.Red);
                GDI32.SaveImage(filtered);
            }

            // parsed = new bool[numWidth, numHeight]; TODO remove
            hints = new int[numWidth, numHeight];
            for (int y = 0; y < numHeight; ++y)
                for (int x = 0; x < numWidth; ++x)
                    hints[x, y] = -1; //default, unknown value
        }
        ...
        public void update() //IMPORTANT--gets a new screenshot
        {
            ...
            // look for a game end textbox
            gameFinished = gameWindowDisplayed();
            //Console.WriteLine("Game finished? {0}", gameFinished);
        }
        ...
        {
            //SHOW code
            //makeImageAt(new Point(leftCol, topHeight), width, height);
            //Bitmap b = new Bitmap(numWidth, numHeight);
            ...
            for (; p.Y < numHeight; p.Y++)
                for (p.X = 0; p.X < numWidth; p.X++)
                    ...
                    {
                        int num = getNum(p);
                        //b.SetPixel(p.X, p.Y, myAverageColor);
                        if (num >= 0 && num < 9)
                        {
                                    ...
                                    ref Point p3, ref int dx3)
        {
            ...
            //if (true)
            //{
            //    Bitmap box = makeImageAt(p, dx, dy);
            //    Bitmap redBox = new Bitmap(box);
            //    for (int x = 0; x < box.Width; ++x)
            //        for (int y = 0; y < box.Height; ++y)
            //            redBox.SetPixel(x, y, (box.GetPixel(x, y).R > 65 ? Color.Red : Color.Black));
            //    GDI32.SaveImage(redBox);

            //    Bitmap topBottomBox = new Bitmap(box);
            //    for (int x = 0; x < box.Width; ++x)
            //        topBottomBox.SetPixel(x, top, Color.LimeGreen);
            //    for (int x = 0; x < box.Width; ++x)
            //        topBottomBox.SetPixel(x, Math.Min(bottom, box.Height - 1), Color.LimeGreen);
            //    GDI32.SaveImage(topBottomBox);

            //    Bitmap leftRightBox = new Bitmap(box);
            //    for (int y = 0; y < box.Height; ++y)
            //        leftRightBox.SetPixel(p1.X, y, Color.Green);
            //    for (int y = 0; y < box.Height; ++y)
            //        leftRightBox.SetPixel(p1.X + dx1, y, Color.Green);

            //    for (int y = 0; y < box.Height; ++y)
            //        leftRightBox.SetPixel(p2.X, y, Color.Blue);
            //    for (int y = 0; y < box.Height; ++y)
            //        leftRightBox.SetPixel(p2.X + dx2, y, Color.Blue);

            //    for (int y = 0; y < box.Height; ++y)
            //        leftRightBox.SetPixel(p3.X, y, Color.Purple);
            //    for (int y = 0; y < box.Height; ++y)
            //        leftRightBox.SetPixel(Math.Min(p3.X + dx3, box.Width - 1), y, Color.Purple);

            //    GDI32.SaveImage(leftRightBox);
            //}
        ...
        //Finds out how many mines are in the game
        private int minesRemaining()
        {
            ...
            //GDI32.SaveImage(num1);
            //GDI32.SaveImage(num2);
            //GDI32.SaveImage(num3);
        ...
        //Searches for a Game End window displayed (for loss or win)
        private bool gameWindowDisplayed()
        {
            int y = topHeight - 10;
            int x = (leftCol + rightCol) / 2;

            Color c = getPixel(x, y);
            if(topColor.Equals(Color.White))
                topColor = c;

            if (!c.Equals(topColor))
            {
                //Console.WriteLine("ERROR: The game window has been moved!!!");
                return true;
            }

            y = (bottomHeight + topHeight) / 2 - 10;
            //The game window is a RGB 240,240,240 uniform box
            for (int i = -5; i < 5; ++i)
            {
                c = getPixel(x + 5 * i, y);
        ...
        private Bitmap makeImageAt(Point p, int sizex, int sizey)
        {
    ...
    //  TODO eventually get rid of old deugging code 
     /* getScreen();
      int SIZE = 650;
